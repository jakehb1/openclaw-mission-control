...
export function useClawdbotAgents() {
  ...
  const gatewayConnected = healthQuery.data?.ok ?? false;
  
  // Show agents as online when gateway is connected
  const agents = useMemo(() => {
    return CLAWDBOT_AGENTS.map(agent => ({
      ...agent,
      // Main agent is online if gateway is connected
      status: gatewayConnected && agent.id === 'main' ? 'online' : 'offline',
      lastSeen: gatewayConnected ? new Date().toISOString() : undefined,
    })) as ClawdbotAgent[];
  }, [gatewayConnected]);
